...
import com.alfredbase.store.sql.OnLinePayLogSQL;
...
import com.puscene.posclient.adapter.EditSettlementAdapter;
import com.puscene.posclient.global.App;
import com.puscene.posclient.global.JavaConnectJS;
import com.puscene.posclient.jobs.CloudSyncJobManager;
import com.puscene.posclient.popupwindow.CloseOrderSplitWindow;
import com.puscene.posclient.popupwindow.CloseOrderWindow;
import com.puscene.posclient.popupwindow.PayOrderWindow;

public class EditSettlementPage extends BaseActivity {
	private String TAG = EditSettlementPage.class.getSimpleName();
	private PayOrderWindow closeOrderWindow;
...
//		setContentView(R.layout.edit_settlement_page);
		addSubView(R.layout.edit_settlement_page, App.instance.isPrinterLink());
		closeOrderWindow = new PayOrderWindow(this,
				findViewById(R.id.rl_root), mHandler, ((TextView)findViewById(R.id.tv_showat)));
		...
		((LinearLayout)findViewById(R.id.ll_print)).setVisibility(View.INVISIBLE);
	}
	...
	private Handler mHandler = new Handler() {
		public void handleMessage(android.os.Message msg) {
			...
			case JavaConnectJS.ACTION_CLICK_EDIT_SETTLEMENT: {
				verifyDialog.show(EDIT_ITEM_ACTION, msg.obj, App.instance.getUser());
			}
				break;
			...
			case MainPage.VIEW_EVENT_CLOSE_BILL: {
				...
				PrinterTitle title = ObjectFactory.getInstance()
						.getPrinterTitle(
								...
								App.instance.getRevenueCenter().getRevName(),
								currentOrder,
...
//				printerLoadingDialog.showByTime(3000);
				...
				for (Map<String, Object> paymentMap : oldPaymentMapList) {
					...
					case ParamConst.SETTLEMENT_TYPE_ALIPAY:
						...
						if (oldAlipaySettlement != null) {
							...
							OnLinePayLogSQL.updateOnLinePayLogStatus(ParamConst.ACTIVE_NOMAL, oldAlipaySettlement.getPayOrderNo());
						}
						break;
					case ParamConst.SETTLEMENT_TYPE_WEIXIN:
						...
						if (oldWeixinSettlement != null) {
							...
							OnLinePayLogSQL.updateOnLinePayLogStatus(ParamConst.ACTIVE_NOMAL, oldWeixinSettlement.getPayOrderNo());
						}
						break;
					default:
							break;
			...
			case MainPage.VIEW_EVENT_SHOW_BILL_ON_HOLD:
				verifyDialog.show(MainPage.HANDLER_MSG_OBJECT_BILL_ON_HOLD, null, App.instance.getUser());
				break;
			case MainPage.VIEW_EVENT_SHOW_VOID:
				verifyDialog.show(MainPage.HANDLER_MSG_OBJECT_VOID, null, App.instance.getUser());
				break;
			case MainPage.VIEW_EVENT_SHOW_ENTERTAINMENT:
				verifyDialog.show(MainPage.HANDLER_MSG_OBJECT_ENTERTAINMENT, null, App.instance.getUser());
				break;
				...
				if (result.get("MsgObject").equals(
					...
					if (!verifyDialog.isShowing()) {
						if(closeOrderWindow.isShowing()){
							...
							closeOrderWindow.initSpecialSettlement(user,
	...
	private void showCloseBillWindow(EditSettlementInfo editSettlementInfo) {
		...
		case ParamConst.BILL_TYPE_UN_SPLIT:
			...
			closeOrderWindow.show(currentOrder, orderBill, ParamConst.SETTLEMENT_TYPE_CASH);
			break;
