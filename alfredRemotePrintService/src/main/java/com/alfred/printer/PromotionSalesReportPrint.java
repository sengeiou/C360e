...
public class PromotionSalesReportPrint extends ReportBasePrint{
	...
	public final static int FIXED_COL3_QTY = 11; //in case of 48 dots width, QTY col = 10dots
	...
	public PromotionSalesReportPrint(String uuid, Long bizDate) {
...
//		getHourlySalesStr();
	...
	private String GetThreeColHeader(String col1Title, String col2Title, String col3Title) {
		...
		PromotionSalesReportPrint.COL3_ITEMNAME = this.charSize -  PromotionSalesReportPrint.FIXED_COL3_QTY - PromotionSalesReportPrint.FIXED_COL3_TOTAL;
		String title1 = StringUtil.padRight(col1Title, PromotionSalesReportPrint.COL3_ITEMNAME);
		String title2 = StringUtil.padRight(col2Title, PromotionSalesReportPrint.FIXED_COL3_QTY);
		String title3 = StringUtil.padLeft(col3Title, PromotionSalesReportPrint.FIXED_COL3_TOTAL);
	...
	private String GetThreeColContent(String col1Content, String col2Content, 
		...
		PromotionSalesReportPrint.COL3_ITEMNAME = (this.charSize -PromotionSalesReportPrint.FIXED_COL3_TOTAL - PromotionSalesReportPrint.FIXED_COL3_QTY)/charScale - 2*PromotionSalesReportPrint.FIXED_COL3_SPACE;
		
		double ln1 = col1Content.length();
		try {
			ln1 = (col1Content.getBytes("GBK").length)/(PromotionSalesReportPrint.COL3_ITEMNAME*1.0);
		} catch (UnsupportedEncodingException e) {
			e.printStackTrace();
			ln1 = col1Content.length();
		}
		col1Lines = StringUtil.nearestTen(ln1);
		String col1PadContent = StringUtil.padRight(col1Content, col1Lines*PromotionSalesReportPrint.COL3_ITEMNAME);
		ArrayList<String> splittedCol1Content = StringUtil.splitEqually(col1PadContent, PromotionSalesReportPrint.COL3_ITEMNAME);
		
		double ln2 = (col2Content.length())/(PromotionSalesReportPrint.FIXED_COL3_QTY*1.0/charScale);
		col2Lines = StringUtil.nearestTen(ln2);
		String col2PadContent = StringUtil.padLeft(col2Content, col2Lines*PromotionSalesReportPrint.FIXED_COL3_QTY/charScale);
		ArrayList<String> splittedCol2Content = StringUtil.splitEqually(col2PadContent, PromotionSalesReportPrint.FIXED_COL3_QTY/charScale);

		double ln3 = (col3Content.length())/(PromotionSalesReportPrint.FIXED_COL3_TOTAL*1.0/charScale);
		col3Lines = StringUtil.nearestTen(ln3);
		String col3PadContent = StringUtil.padLeft(col3Content, col3Lines*PromotionSalesReportPrint.FIXED_COL3_TOTAL/charScale);
		ArrayList<String> splittedCol3Content = StringUtil.splitEqually(col3PadContent, PromotionSalesReportPrint.FIXED_COL3_TOTAL/charScale);
		...
		for (int i=0; i< Math.max(Math.max(col1Lines, col2Lines),col3Lines); i++) {
			if (i<col1Lines) {			
				...
				result.append(StringUtil.padRight(" ", PromotionSalesReportPrint.COL3_ITEMNAME));
			}
			//padding
			result.append(StringUtil.padRight(" ", PromotionSalesReportPrint.FIXED_COL3_SPACE));
			
			if (i<col2Lines) {
				result.append(splittedCol2Content.get(i));
			}else {
				result.append(StringUtil.padLeft(" ", (PromotionSalesReportPrint.FIXED_COL3_QTY)/charScale));
			}
			...
			if (i<col3Lines) {
				...
				result.append(StringUtil.padLeft(" ", (PromotionSalesReportPrint.FIXED_COL3_TOTAL)/charScale));
			}			
