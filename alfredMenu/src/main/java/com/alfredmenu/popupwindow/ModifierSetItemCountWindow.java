...
public class ModifierSetItemCountWindow implements OnClickListener, MoneyKeyboard.KeyBoardClickListener {
	...
	public ModifierSetItemCountWindow(BaseActivity parent, View parentView, Handler handler) {
		this.parent = parent;
	...
	public void show(int count, ItemDetail itemDetail, ModifierVariance modifierVariance) {
		init();
			...
			set.addListener(new AnimatorListenerImpl() {
				public void onAnimationEnd(Animator animation) {
					...
					if(isEnter){
						...
						modifierVariance.setModQty(Integer.parseInt(tv_num.getText().toString()));
						result.put("itemDetail", itemDetail);
						result.put("modifierVariance", modifierVariance);
						result.put("count", Integer.parseInt(tv_num.getText().toString()));
						handler.sendMessage(handler.obtainMessage(
								MainPage.VIEW_EVENT_MODIFIER_COUNT, result));
					}
				};
			});
			set.start();
