...
public class GalleryFinal {
    ...
    public static void openGallerySingle(int requestCode, OnHanlderResultCallback callback) {
        ...
        if (config != null) {
            ...
            if(callback != null) {
                callback.onHanlderFailure(requestCode, mCoreConfig.getContext().getString(cn.finalteam.galleryfinal.R.string.open_gallery_fail));
            }
    ...
    public static void openGallerySingle(int requestCode, FunctionConfig config, OnHanlderResultCallback callback) {
        ...
        if (!DeviceUtils.existSDCard()) {
            Toast.makeText(mCoreConfig.getContext(), cn.finalteam.galleryfinal.R.string.empty_sdcard, Toast.LENGTH_SHORT).show();
            return;
        }
    ...
    public static void openGalleryMuti(int requestCode, FunctionConfig config, OnHanlderResultCallback callback) {
        ...
        if ( config.getMaxSize() <= 0) {
            if(callback != null){
                callback.onHanlderFailure(requestCode, mCoreConfig.getContext().getString(cn.finalteam.galleryfinal.R.string.maxsize_zero_tip));
            }
            return;
        }
        ...
        if (config.getSelectedList() != null && config.getSelectedList().size() > config.getMaxSize()) {
            if(callback != null){
                callback.onHanlderFailure(requestCode, mCoreConfig.getContext().getString(cn.finalteam.galleryfinal.R.string.select_max_tips));
            }
            return;
        }
