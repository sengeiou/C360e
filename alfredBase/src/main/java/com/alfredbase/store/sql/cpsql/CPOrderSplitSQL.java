...
public class CPOrderSplitSQL {
	...
	public static OrderSplit get(Integer id) {
		...
		String sql = "select * from " + TableNames.CPOrderSplit + " where id = ?";
		Cursor cursor = null;
		try {
	...
	public static int getOrderSplitsCountByOrder(Order order) {
		String sql = "select count(*) from " + TableNames.CPOrderSplit
				+ " where orderId = ?";
		Cursor cursor = null;
		int orderQty = 0;
		SQLiteDatabase db = SQLExe.getDB();
...
//				+ " ))";
	...
	public static void updateOrderSplitStatus( int orderStatus, int id){
		...
		String sql = "update " + TableNames.CPOrderSplit + " set orderStatus = ? where id = ?" ;
		try {
