...
public class RestaurantConfigSQL {
    ...
    public static List<RestaurantConfig> getAllRestaurantConfigByParaType(int paraType) {
        ArrayList<RestaurantConfig> result = new ArrayList<>();
        String sql = "select * from " + TableNames.RestaurantConfig + " where paraType = ?";
        Cursor cursor = null;
        SQLiteDatabase db = SQLExe.getDB();
        try {
            cursor = db.rawQuery(sql, new String[]{String.valueOf(paraType)});
            int count = cursor.getCount();
            if (count < 1) {
                return result;
            }
