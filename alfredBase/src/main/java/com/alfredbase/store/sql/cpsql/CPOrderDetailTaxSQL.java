...
public class CPOrderDetailTaxSQL {
	...
	public static void updateOrderDetailTax(SQLiteDatabase db, OrderDetailTax orderDetailTax) {
		if (orderDetailTax == null) {
			return;
		}
		try {
...
//		String sql ="select sum(ot.taxPrice), ot.taxName, ot.taxPercentage from " + TableNames.CPOrderDetailTax + " ot, " + TableNames.TaxCategory + " tc" + " where ot.orderId = ? and ot.taxId = tc.taxId and tc.status = 1 group by ot.taxId order by tc.indexId";
		String sql ="select sum(ot.taxPrice), ot.taxName, ot.taxPercentage from " + TableNames.CPOrderDetailTax + " ot where ot.orderId = ? and ot.isActive <> " + ParamConst.ACTIVE_DELETE + " and  ot.taxId in ( select taxId from " + TableNames.TaxCategory + " where status = 1) group by ot.taxId order by ot.indexId";
		Cursor cursor = null;
		SQLiteDatabase db = SQLExe.getDB();
		try {
...
//		map.put("taxPercentages", taxPercentages);
	...
	public static Map<String, Object> getTaxDetail(long businessDate) {
		...
		try {
			String sql = "select sum(taxPrice), taxName, taxPercentage, taxId, count(*) from "
					...
					+ TableNames.OrderDetail
					+ " od";
			cursor = db.query(TableNames.CPOrderDetailTax,
	...
	public static ArrayList<OrderDetailTax> getAllOrderDetailTax(Order order) {
		...
		String sql = "select * from " + TableNames.CPOrderDetailTax
	...
	public static void updateOrderDetailTaxActiveByOrder(int isActive, int orderId){
		String sql = "update " + TableNames.CPOrderDetailTax + " set isActive = ? where orderId = ?" ;
		if(isActive == ParamConst.ACTIVE_REFUND){
			sql = "update " + TableNames.CPOrderDetailTax + " set isActive = ? where orderId = ? and isActive <> " + ParamConst.ACTIVE_DELETE;
		}
		try {
	...
	public static void updateOrderDetailTaxActiveByOrderSplit(int isActive, int orderSplitId){
		String sql = "update " + TableNames.CPOrderDetailTax + " set isActive = ? where orderSplitId = ?" ;
		if(isActive == ParamConst.ACTIVE_REFUND){
			sql = "update " + TableNames.CPOrderDetailTax + " set isActive = ? where orderSplitId = ? and isActive <> " + ParamConst.ACTIVE_DELETE;
		}
		try {
	...
	public static void updateOrderSplitIdbyOrderDetail(OrderDetail orderDetail){
		...
		String sql = "update " + TableNames.CPOrderDetailTax + " set orderSplitId = ? where orderDetailId = ?" ;
		try {
	...
	public static void updateOrderDetailTaxForTransation(OrderDetail newOrderDetail, OrderDetail oldOrderDetail){
		String sql = "update " + TableNames.CPOrderDetailTax + " set orderId = ? , orderDetailId = ? ,orderSplitId = ? where orderDetailId = ?";
		try {
	...
	public static void deleteOrderDetailTax(OrderDetailTax orderDetailTax) {
		String sql = "delete from " + TableNames.CPOrderDetailTax
				+ " where id = ?";
		try {
			SQLExe.getDB()
